...
if(entity_1 ~= 0 and entity_2 ~= 0)then
	...
	--	print("eee?")
		...
		local x1, y1 = EntityGetTransform(entity_1)

		local x2, y2 = EntityGetTransform(entity_2)

		local x, y = EntityGetTransform(rope)

		--GamePrint("X: "..tostring(x1)..", Y: "..tostring(y1))

		--EntityLoad("data/entities/misc/debug_marker.xml", x1, y1)
		--EntityLoad("data/entities/misc/debug_marker.xml", x2, y2)

		--EntityLoad("data/entities/misc/debug_marker.xml", x, y)
		--EntitySetTransform(rope, x2, y2)

		y2 = y2 - 4

		--x1 = x1 + 18

		EntitySetVerletPoints(rope, x1, y1, x2, y2)
		...
		edit_component( rope, "VerletPhysicsComponent", function(comp,vars)
			...
			--print(table.dump(positions))

			positions[(last_point_index * 2) - 3] = x2
			positions[(last_point_index * 2) - 2] = y2

			--print("Index = "..last_point_index * 2)

			--print(table.dump(positions))

			ComponentSetValue2(comp, "positions", positions)
		end)
		]]
